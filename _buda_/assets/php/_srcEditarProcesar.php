...
foreach ($setDatos as $dato) {
    ...
    switch($tipo){
        case 'imagen':
            $vericar.=  '$'.$nombre.' =$_POST["ctrTmp'.$nombre.'"] ?? \'\';'. PHP_EOL;
            //$vericar.=  '$_POST["ctr'.$nombre.'"] ?? \'\';'. PHP_EOL;

            $scriptUploadSingle.='if(empty($nombre)){  
                                    if (isset($_FILES["ctr'.$nombre.'"]) && $_FILES["ctr'.$nombre.'"]["error"] === 0) {
...
$sqlCamposLet.='si';
$sql.=  $sqlCampos.",actualizado=? WHERE id=?\";";
            ...
            foreach ($_FILES["imagenes"]["tmp_name"] as $key => $tmp_name) {
               ...
               $rutaArchivo = $uploadDir . $nombreUnico;
                ...
                if (move_uploaded_file($tmp_name, $rutaArchivo)) {
                    ...
                    $stmtImg->bind_param("iss", $id, $nombreUnico, $portada);
                    if (!$stmtImg->execute()) {
                        unlink($rutaArchivo);
...
<?php
    ...
    try {
        ...
        \$sqImg=mysqli_query(\$cnn,"SELECT * FROM $setTablaImagenes WHERE $setCampoImgen=".\$id);
        while(\$rwImg=mysqli_fetch_array(\$sqImg)){
            \$getIdformIg    = \$rwImg['id'];
            \$archivo        = \$baseUploadImagenes.\$rwImg['imagen'];
            \$getImagenStatus= \$_POST["multimagen-".\$getIdformIg] ?? 'true';
            if(\$getImagenStatus=='false'){
                mysqli_query(\$cnn,"UPDATE $setTablaImagenes SET estado='ELIMINADO' WHERE id=".\$getIdformIg);
