...
$(document).ready(function(){
    ...
    $('.preview-img-add').click(function(){
        $("#ctrImaproductos").click();
    })

    $('.preview-img-loaded').each(function() {
        $(this).click(function () {
            $("#dropArea").hide();
            $("#divPreviewContenedor").css("display", "flex");
            $("#previewArea").html("<img src='" + $(this).attr("src") + "'>");
            const setImgId=$(this).attr("id");
            const setImgIdsOLO=$(this).attr("data-setid");
            $("#previewAreaOptions").html("<i onclick=deleteImagenExistente('"+setImgId+"','"+setImgIdsOLO+"') class='menu-icon size22 mdi mdi-trash-can-outline'></i><br/><span onclick=setMarcaPortada('TRUE','a') class='mdi colorBlueSel size22 mdi-star-box'></span>");
            $("#previewArea").removeClass("hide-before");
        });
    });
...
function deleteImagenExistente(divImg, divSetId){
    $('#'+divSetId).val('false');
    $('#'+divImg).remove();
    $("#dropArea").show();
    $("#divPreviewContenedor").css("display", "none");
